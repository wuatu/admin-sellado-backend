                ...
                if (option == '1') {
                    ...
                    if (date && time && id_caliper) {
                        ...
                        if (hora == '00') {
                            ...
                            searchBox = yield database_1.default.query('SELECT COUNT(DISTINCT(codigo_de_barra)) AS total FROM registro_diario_caja_sellada WHERE fecha_sellado = ? AND hora_sellado >= ? AND hora_sellado < ? AND id_calibrador = ?', [date, horaMenosUna, '23:59:59', id_caliper]);
                            searchBoxAux = yield database_1.default.query('SELECT COUNT(DISTINCT(codigo_de_barra)) AS total FROM registro_diario_caja_sellada WHERE fecha_sellado = ? AND hora_sellado >= ?  AND id_calibrador = ?', [fecha_actual, '00:00:00', id_caliper]);
                            searchBox[0].total = searchBox[0].total + searchBoxAux[0].total;
