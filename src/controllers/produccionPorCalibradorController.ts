...
class ProduccionPorCalibradorController {
    ...
    public async countBoxInCaliper(req: Request, res: Response) {
        try {
            ...
            if (id_caliper ) 
            {
                numberBox = await pool.query('SELECT fecha_sellado, Count(fecha_sellado) as numero FROM registro_diario_caja_sellada WHERE id_calibrador = ? AND (fecha_sellado BETWEEN ? AND ?) group by fecha_sellado', [id_caliper, fromDateSearch, toDateSearch]);
