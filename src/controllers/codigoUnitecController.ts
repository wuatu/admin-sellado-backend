...
class CodigoUnitecController {
    ...
    public async list(req: Request, res: Response) {
        try {
            ...
            registrosDev = await pool.query('SELECT DISTINCT cod_Caja_Unitec AS cod_caja, Codigo_Confection_Unitec AS codigo_confection, Confection_Unitec AS confection, Codigo_Embalaje_Unitec AS codigo_embalaje, Embalaje_Unitec AS embalaje, Codigo_Envase_Unitec AS codigo_envase, Envase_Unitec AS envase, Categoria_Unitec AS categoria, Categoria_Timbrada_Unitec AS categoria_timbrada  FROM registro_diario_caja_sellada ORDER BY id DESC LIMIT 100');
